...
Compiler.prototype.getFiles = function() {
  var self = this;
  var deferred = q.defer();
  recursive(self.config.watchDir, function(err, files) {
    if (err) {
      deferred.reject(err);
    } else {
...
Compiler.prototype.compile = function() {
  ...
  return q.all([self.getFiles(), self.getGiderosConfigData()]).then(function(res) {
    ...
    var tree = {};
    _.each(includeFiles, function(f) {
    	tree[f] = f.split(path.sep);
    });
    tree = self.buildGiderosJsonTree(tree);
